...
(*      Matrix`justUpdated$$ = True; Matrix`userTry$$ = True; *)
(*       Matrix`showErrors$$ = False; Matrix`currentElement$$ = 0; *)
...
(*    If[Dynamic[ !Matrix`userTry$$], Matrix`seed$$ = ""; *)
(*      Matrix`justUpdated$$ = False; Matrix`matriceA$$ = *)
(*       ConstantArray[0, {Matrix`rowsA$$, Matrix`colA$$}]; *)
...
(*                     Center, Appearance -> Dynamic[If[Matrix`userTry$$, *)
(*                       Frameless]], Enabled -> Dynamic[ !Matrix`userTry$$]]], *)
(*                  {Matrix`i, Matrix`rowsA$$}, {Matrix`j, Matrix`colA$$}]], *)
(*                Frame -> All, Spacings -> {1, 1}, ItemSize -> {3, 2}]]}, *)
(*             Alignment -> Center], Spacer[20], Column[{"*"}]}], *)
(*          Row[{Column[{"Matrice B:", Dynamic[Grid[If[Matrix`randomFill, *)
(*                 Matrix`matriceB$$, Print[Matrix`userTry$$]; Table[*)
(*                   With[{Matrix`i$ = Matrix`i, Matrix`j$ = Matrix`j}, *)
(*                    InputField[Dynamic[Matrix`matriceB$$[[Matrix`i$,*)
(*                       Matrix`j$]]], Number, FieldSize -> {3, 1}, *)
(*                     Alignment -> Center, Appearance -> Dynamic[*)
(*                       If[Matrix`userTry$$, Frameless]], Enabled -> Dynamic[*)
(*                        !Matrix`userTry$$]]], {Matrix`i, Matrix`rowsB$$}, *)
(*                   {Matrix`j, Matrix`colB$$}]], Frame -> All, Spacings -> *)
(*                 {1, 1}, ItemSize -> {3, 2}]]}, Alignment -> Center], *)
(*            Spacer[20], Column[{"="}]}]}], *)
(*       Column[{Spacer[50], Button["Inizia", Matrix`userTry$$ = True, *)
(*          ImageSize -> 60], Spacer[50], "Matrice A*B:", *)
...
(*              Matrix`currentElement$$++]], Button["Torna indietro", *)
(*             If[Matrix`currentElement$$ > 0, Matrix`currentElement$$--]]}]]}, *)
(*        Alignment -> Center]}, Alignment -> Center], *)
(*     Row[{Button["Verifica Risultato", If[Matrix`currentElement$$ != *)
(*          Dimensions[Matrix`matriceAB$$][[1]]*Dimensions[Matrix`matriceAB$$][[*)
(*            2]], Matrix`showErrors$$ = True; Matrix`currentElement$$ = *)
(*           Dimensions[Matrix`matriceAB$$][[1]]**)
(*            Dimensions[Matrix`matriceAB$$][[2]]; ]], Spacer[10], *)
(*       Button["Mostra Soluzione", Matrix`currentElement$$ = *)
(*          Dimensions[Matrix`matriceAB$$][[1]]*Dimensions[Matrix`matriceAB$$][[*)
(*            2]]; ], Spacer[10], Button["Reset", Matrix`userTry$$ = False; *)
(*         Matrix`showErrors$$ = False; Matrix`currentElement$$ = 0; *)
...
(*           {Matrix`rowsB$$, Matrix`colB$$}]; Matrix`matriceAB$$ = *)
(*          Matrix`matriceA$$ . Matrix`matriceB$$; Matrix`inputUtente$$ = *)
