...
GeneraInterfaccia::usage="generaInterfaccia [] 
>>>>>>> c8350cbdee84497fbba6ec7aafd304b0424a3f7d
...
GeneraInterfaccia[]:= DynamicModule[{
    ...
    (*Caso randomico*)
       If[randomFill && seed!="",
		...
		(*Controllo che il numero che l'utente ha inserito sia un intero e sia un numero compreso tra 1 e 6,
			se non lo è viene creata una finestra in cui viene dato un errore all'utente*)
	                If[IntegerQ[#] && 1 <= # <= 6 , 
            ...
            Column[{
	            Row[Spacer[10]{
		            Row[{
						...
						(*I numeri che l'utente può inserire nelle matrici (A e B) devono stare dentro al range di -9999 e 9999,
				...
				Dynamic@Column[{
					...
					(*Se le colonne della matrice A sono uguali alle righe della matrice B, allora è possibile fare il prodotto tra le due matrici*)
						Button["Inizia", 
							If[colA == rowsB,
	...
	dimensioni uguali per poter generare una matrice", FontFamily -> "Helvetica", TextAlignment->Center, FontColor->Red]], Background->LightGray],
	                ...
	                Spacer[80],
	                Column[{
			(*I bottoni risolvi precedente e risolvi successivo sono una "pseudo guida" per l'utente in cui si evidenziano le righe e le colonne delle matrici e la casellina corrispondente nella matrice AB in cui andrà inserito il calcolo.
